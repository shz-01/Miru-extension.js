...
export default class extends Extension {
  ...
  async createFilter() {
    const filter = {
      ...
      sorts: {
        ...
        default: "",
        options: {
          "time": "时间排序", "hits": "人气排序", "score": "评分排序"
        }
      }
    }
    return filter
  ...
  // 动漫筛选功能 Anime Filter Function
  async filter(page, filter) {
    const res = await this.request(`/show/by/${filter?.sorts}/id/1/page/${page}/letter/${filter?.letters}/year/${filter?.years}.html`)
    const bangumiList = await this.querySelectorAll(res, 'div[class="module-items module-poster-items-base "] a')
    const bangumi = []
    for (let i = 0; i < bangumiList.length; i++) {
      const title = await this.queryXPath(bangumiList[i].content, '//div[@class="module-poster-item-title"]').text
  ...
  // 动漫搜索功能 Anime Search Function
  async search(kw, page, filter) {
    // if (filter?.sorts) { return await this.filter(page, filter) } 此代码与lastest冲突 问题：调用筛选功能（未选中）导致最新功能只显示一页
    // if (filter?.sorts) { return await this.filter(page, filter) }
